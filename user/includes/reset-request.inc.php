...
function checkSecurityQuestions($connection, $email, $securityQuestion1, $securityQuestion2, $securityQuestion3, $securityAnswer1, $securityAnswer2, $securityAnswer3) {
    $sql = "SELECT person.PERSON_ID, security_questions.PERSON_ID, SECURITY_QUESTION_1, SECURITY_ANSWER_1, SECURITY_QUESTION_2, SECURITY_ANSWER_2, SECURITY_QUESTION_3, SECURITY_ANSWER_3
    FROM person
    JOIN security_questions ON person.PERSON_ID = security_questions.PERSON_ID
...
if (isset($_POST["reset-request-submit"])){
    ...
    $url = "http://localhost/capstone/user/create-new-password.html?selector=" . $selector . "&validator=" . bin2hex($token) . "&email=" . $userEmail;
